...
class DeepImagePrior(object):
    ...
    def DIP(self,Y,H):
        ...
        for bs in range(batch_size):
            ...
            net_input = torch.randn(1,4).to(device) ### Random input for DIP
...
class Decoder(nn.Module):
    def __init__(self,num_tx_ant):
        ...
        self.nn1 = nn.Linear(4,8).to(device) ### Define numbers of input (4) and output (8) of fully connected layer
                                             ### Weight.shape = (number of output (8),number of input (4))
                                             ### Bias.shape = number of output (8)
        self.nn2 = nn.Linear(8,16).to(device)
        self.nn3 = nn.Linear(16,32).to(device)
        self.nn4 = nn.Linear(32,(2*num_tx_ant)).to(device)
        self.act = nn.Tanh().to(device)      ### Define Activation Function: Tanh()
         
    def forward(self,x): 
        o1 = self.act((self.nn1(x)).to(device)) 
