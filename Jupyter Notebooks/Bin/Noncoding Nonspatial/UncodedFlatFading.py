    ...
    def __call__(self,
        ...
        for i in range(0, NUM_DATA_GROUP):
            ...
            for EBN0_DB in np.linspace(EBN0_DB_MIN,EBN0_DB_MAX,NUM_EBN0_POINTS):
                ...
                x_real_part = np.real(x_reshape)
                x_image_part = np.imag(x_reshape)
                x_real = np.concatenate([x_real_part,x_image_part], axis=1)

                y_real_part = np.real(y)
                y_image_part = np.imag(y)
                y_real = np.concatenate([y_real_part,y_image_part], axis=1)

                h_real_part = np.real(h)
                h_imag_part = np.imag(h)
                h_real = np.concatenate([np.concatenate([h_real_part, -h_imag_part], axis=2), np.concatenate([h_imag_part, h_real_part], axis=2)],axis=1)
